...
**`engine/stages/` is the wiring layer and only the wiring layer.** It holds one `run(ctx, rulepack, run_config)` adapter module per entry in `engine/registry.py` — 58–262 lines each — and nothing else, so the registry reads as a literal table of contents for the directory. The *domains* those adapters drive are peer packages under `engine/`: `hierarchy/`, `classify/`, `re_split/`, `scope/`, `fx/`, alongside `crm/`, `sa/`, `irb/`, `slotting/`, `ccr/`, `sft/`, `cva/`, `equity/`, `securitisation/`, `aggregator/`, `kernels/`. Import `HierarchyResolver` from `engine.hierarchy`, `ExposureClassifier` from `engine.classify`, `RealEstateSplitter` from `engine.re_split`, `FXConverter` from `engine.fx`.

**This reverses an earlier deliberate choice — do not "fix" it back.** Those five packages used to live *inside* `engine/stages/`, which meant one directory name covered two structurally different things: a thin adapter for 7 of 11 stages, and the entire 744–3,943 line domain for the other 4 — plus `fx`, which is not a registry stage at all (no `run`, no registry slot; it is called from inside `engine/hierarchy/resolver.py`). A reader could not tell which was which without opening the files, and the stage↔domain map is not 1:1 and never can be (`calculators` drives SA, IRB and slotting from one adapter). The move is recorded in `docs/plans/architecture-review-2026-08-29.md` §2; `arch_check` check 16 now fails any package that reappears under `engine/stages/` **without a `run`** — a package that reappears *with* one is a legal stage package and is permitted by design.

What the move did **not** change: each package is still the **single home** of its component — there is no top-level alias module. When an implementation moves, delete the old module and repoint its importers; never leave a back-compat shell behind (arch_check check 18). The four shells the Phase 4 slices left behind were deleted once one of them was found silently disarming a regression guard that read `module.__file__`.
...
- **Architecture**: `uv run python scripts/arch_check.py` — the 22 numbered invariants, plus `watchfire check` as its final step.
...
- **Reporting coverage ratchet**: `uv run python scripts/coverage_report.py --check` (~40s, 18 pipeline runs). A four-way ratchet on how much of the estate the gates can SEE — binding rules per regime and template-cell liveness may not fall, dead cells and never-evaluated rules may not rise. **Nothing else runs it.** It is its own CI job (`Reporting Coverage Ratchet`), it is not in `arch_check`, and its local mirror `tests/contracts/test_coverage_ratchet.py` is `slow`-marked, so the dev-loop `pytest` line below AND CI's own `test` job both deselect it. A change that moves a reported row or column can therefore be green everywhere else and red in CI. Bank a deliberate move with `--update-baseline` — re-measure it, never type the numbers — and say why in `_note` and the commit message.
...
plan item. `scripts/arch_check.py`'s 22 numbered checks and the supervisory
