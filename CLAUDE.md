...
The batch commands pick N non-conflicting items and run the validation gate (or `uv run zensical build`) **once at the end** — not per agent. `/next-items` provisions one git worktree per item on `batch/<batch-id>/<P-code>` and drives the four-wave pipeline directly from the orchestrator session, dispatching one role-agent per surviving item per wave (parallel within a wave, sequential between waves); the orchestrator then squash-merges the worktree branches into the current feature branch before the gate runs. `/next-docs` keeps the flat parallel `doc-writer` dispatch (no worktree — docs writes don't collide). Items that touch shared engine files (`engine/pipeline.py`, `contracts/protocols.py`, `contracts/bundles.py`, `engine/aggregator/aggregator.py`) are forced single-stream by `/next-items` even when N>1 was requested, and run in the main tree without worktree machinery.

Plus `/implement-scenario <ID>` for ad-hoc one-off work on a specific P-code or scenario ID.

Agents never commit or push — commits land in the slash-command orchestrator only. The call graph is uniformly one level deep (orchestrator → role-agent); sub-agents do not spawn other sub-agents. Claude Code does not propagate the project's `.claude/agents/` registry into sub-sessions, so a nested Agent call from a sub-agent cannot dispatch project role-agents — keep all dispatch in the slash-command orchestrator. The two root plan files (`IMPLEMENTATION_PLAN.md`, `DOCS_IMPLEMENTATION_PLAN.md`) are the source of truth for outstanding work; `docs/plans/implementation-plan.md` is published narrative on the Zensical site.
