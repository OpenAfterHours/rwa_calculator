...
Last updated: 2026-04-08 (D1.24: fixed unrated institution "due diligence approach" → 40% Art. 121 sovereign-derived in crr.md and institution.md)
...
- Art. 132 (CIU) omitted from UK CRR by SI 2021/1078 (1 Jan 2022), moved to PRA Rulebook (CRR Part). **D1.20 analysis needs revision:** CRR2 (Regulation 2019/876) already set Art. 132(2) fallback to **1,250%** (not 100% as originally claimed). The 100% may refer to the original CRR 2013 pre-CRR2 text. Under UK CRR (which incorporates CRR2), the pre-omission fallback was likely 1,250%. Basel 3.1 Art. 132(2) restates 1,250%. The `key-differences.md` "Unchanged" label may be approximately correct for the 1,250% value, but incorrect about Art. 132 status (omitted from CRR, moved to PRA Rulebook). Code uses 150% for CRR CIU fallback (equity-approach.md and sa-risk-weights.md spec also say 150%) — this conflicts with CRR2's 1,250% and needs investigation.
