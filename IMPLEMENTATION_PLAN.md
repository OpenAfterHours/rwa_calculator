...
- **P1.148** [ ] **NEW** — Art. 124I(3) junior-charge CRE >80% LTV double-multiplication. `src/rwa_calc/engine/sa/b31_risk_weights.py:88–93, 503–512, 737–743`. Over-capital +13.75pp on income-producing CRE with >80% LTV. Fix: apply 137.5% as absolute override, not 1.375× multiplier on a 110% base. **Effort: S** | Ref: PRA PS1/26 Art. 124I(3)
...
- **P1.152** [ ] **NEW** — F-IRB repo maturity fixed at 2.5y. `src/rwa_calc/engine/irb/namespace.py:278`. Art. 162(1) requires M = 0.5y for repo-style SFTs. **Effort: S** | Ref: CRR Art. 162(1)
...
- **P1.135** [ ] **NEW (Phase 9)** — FX haircut on collateral silently zero after FX conversion. `engine/crm/haircuts.py:152` — After `FXConverter.convert_collateral()` and `convert_exposures()` both rewrite `currency` to base_currency (GBP), the H_fx comparison `currency != exposure_currency` is always false. Guarantees preserve `original_currency` (fx_converter.py:274); collateral does not (fx_converter.py:164-230). **Art. 224 8% H_fx is NEVER applied to FX-mismatched secured exposures. HIGH capital understatement.** **Effort: S** | Ref: CRR Art. 224, PRA PS1/26 Art. 224
- **P1.136** [ ] **NEW (Phase 9)** — FXConverter preserves `original_currency` only for exposures/guarantees, not collateral/provisions/equity. `engine/fx_converter.py:103, 274` have the aliasing; lines 164-230, 296-424 do not. Root cause of P1.135 and prevents any downstream FX-mismatch check on collateral/provision/equity rows. **Effort: S** | Ref: CRR Art. 224
...
**Open P1 items: 42 total (28S + 12M + 1L + 1 verification).** P1.92 fixed in v0.1.194. Phase 9 added 13 new P1 items (P1.135-P1.147). Highest priority (Phase 9): **P1.135 (FX haircut on collateral silently zero — S, HIGH capital understatement; all FX-mismatched secured exposures)**, **P1.137 (equity transitional Rule 4.2/4.3 SA ladder — M, 2027-2029 capital overstatement for SA-only firms)**, **P1.140 (ADC classification flag-only — M, mis-tagging silently routes development loans to 75-100%)**, **P1.142 (Art. 124E three-property limit not automated — M)**, **P1.141 (Art. 124(4) mixed RE splitting — M)**. Remaining priority (prior phases): **P1.123 (FCCM HE for SFTs — M, capital understatement)**, **P1.122 (guarantee substitution no B31 branching — M, multiple incorrect RWs)**, **P1.130 (aggregator summaries pre-floor — S, reporting inconsistency)**, **P1.109 (guarantee maturity mismatch — M)**, **P1.124 (guarantee eligibility rejection — S)**, **P1.128 (SCRA trade finance <=6m — S)**, P1.111 (RRE junior multiplier capped — S), P1.93 (core market participant — M), P1.95 (guarantor SCRA — M, subsumed by P1.122), P1.96 (covered bond haircut — S, DOWNGRADED), P1.97 (B31 slotting subgrade — S), P1.98 (subordinated LGD floor — S), P1.99 (CRR short-term institution — S), P1.100 (ECA score — S), P1.101 (non-daily revaluation — S), P1.103 (Table 6A — S), P1.104 (FCSM maturity — S), P1.105 (Table 4A — S), P1.106 (FCSM institution UK — S), P1.107 (FCSM B31 corp CQS 3 — S), P1.108 (CRR retail 1.06 — S, DISPUTED), P1.110 (guarantee CQS table — S, subsumed by P1.122), P1.112 (PSE/RGLA sovereign — S), P1.114 (classifier null — S), P1.117 (HVCRE subgrades — S), P1.118 (Art. 162(3) maturity — M), P1.121 (CRR unrated inst short-term — S), P1.125 (FSE column warning — S), P1.126 (null revenue — S), P1.127 (Pool B composition — S, verification), P1.133 (short-term original maturity — S), P1.136 (FXConverter original_currency coverage — S), P1.138 (equity Rule 4.6 IRB higher-of — M), P1.139 (equity Rule 4.8 CIU higher-of — M), P1.143 (Rule 4.11 unfunded CP grandfathering — M), P1.144 (EL fallback ead_final — S), P1.145 (classifier duplicate-permission non-determinism — S), P1.146 (is_guaranteed null drops rows — S), P1.147 (ValidationRequest model_permissions not required — S), P1.30(e) (tranching — L), P1.10 (transitional — M).
...
- **Impact:** **HIGH capital understatement.** `FXConverter.convert_collateral()` and `convert_exposures()` both rewrite the `currency` column to base_currency (GBP) before the haircut calculator runs. `engine/crm/haircuts.py:152` compares `currency != exposure_currency` to decide whether to apply H_fx — the comparison is ALWAYS false post-conversion, so **Art. 224 8% H_fx is never applied to FX-mismatched secured exposures**. Guarantees correctly preserve `original_currency` at `fx_converter.py:274`; collateral does not (`fx_converter.py:164-230`).
- **File:Line:** `engine/crm/haircuts.py:152` (comparison expression), `engine/fx_converter.py:164-230` (missing `original_currency` aliasing for collateral)
- **Spec ref:** CRR Art. 224, PRA PS1/26 Art. 224
- **Fix:** (1) In `fx_converter.py` collateral conversion (lines 164-230), mirror the guarantee aliasing at line 274 — add `collateral_original_currency` (or equivalent) before overwriting `currency`. (2) In `haircuts.py:152`, change the comparison from `currency != exposure_currency` to `collateral_original_currency != exposure_original_currency`. (3) Repeat for provisions and equity currency fields (see P1.136).
- **Tests needed:** Unit test with GBP-based calc, USD exposure + EUR collateral, asserting 8% H_fx applied after FX conversion; regression acceptance test using an existing CRR/B31 acceptance fixture amended to have FX-mismatched collateral.
