...
- **P2.20** [x] **FIXED v0.2.30** — `OutputFloorSummary.total_rwa_post_floor` renamed to `floored_modelled_rwa` (modelled-only IRB+slotting, arithmetic unchanged). New `sa_rwa_total: float = 0.0` and `equity_rwa_total: float = 0.0` fields added; `total_rwa_post_floor` redefined as `floored_modelled_rwa + sa_rwa_total + equity_rwa_total` for genuine portfolio-total semantics per Art. 92(2A). New `SA_APPROACHES` and `EQUITY_APPROACHES` frozensets in `engine/aggregator/_schemas.py` (allowlisted in `arch_check.py` alongside existing `IRB_APPROACHES`). `engine/aggregator/_floor.py` populates new fields at both summary-construction sites via private `_portfolio_sa_equity_totals(combined)` helper — no `aggregator.py` edit required. Consumer migrations in 5 test files (`test_of_adj.py`, `test_portfolio_level_floor.py`, `test_output_floor_skip_transitional.py`, `test_corep.py`, `test_stress_pipeline.py`). Pinned by `tests/unit/test_p2_20_total_rwa_post_floor_naming.py` (9 tests: dataclass field existence + algebraic identity + SA/equity totals from synthetic LazyFrame + load-bearing pure-value test 280+100+30=410). | Ref: PRA PS1/26 Art. 92(2A); `engine/aggregator/_floor.py`
...
- **P2.36** [x] **FIXED v0.2.30** — `PDFloors` dataclass now exposes explicit `sovereign: Decimal` and `institution: Decimal` fields (`src/rwa_calc/contracts/config.py`). `.basel_3_1()` factory returns `Decimal("0.0005")` per Art. 160(1) PRA 0.05% floor; `.crr()` returns `Decimal("0.0003")` per CRR uniform 0.03%. `PDFloors.get_floor()` dispatches sovereign/institution before the corporate fallback. `engine/irb/formulas.py::_pd_floor_expression` extended with two new `pl.when(exposure_class == ...)` branches (CENTRAL_GOVT_CENTRAL_BANK, INSTITUTION) inserted before the final `.otherwise(corporate)` — and added to the all-equal optimisation set. Pinned by `tests/unit/config/test_p2_36_sovereign_institution_pd_floors.py` (14 tests: field existence on both factories + `get_floor` dispatch + load-bearing override-regression where overriding `pd_floors.sovereign=0.001` via `dataclasses.replace` drives RW from 0.174677 → 0.263591, proving dispatch is independent of corporate fallback). | Ref: PRA PS1/26 Art. 160(1); CRR Art. 160(1)
