...
- **P1.103** [x] **FIXED v0.2.13** — PRA PS1/26 Art. 122(3) Table 6A short-term corporate ECAI risk weights implemented. New `B31_CORPORATE_SHORT_TERM_ECAI_RISK_WEIGHTS` constant in `data/tables/b31_risk_weights.py` (CQS1=20%, CQS2=50%, CQS3=100%, Others=150%); new `_b31_append_corporate_maturity_branches` helper in `engine/sa/namespace.py` invoked alongside the existing institution Table 4A branch. Reuses the `has_short_term_ecai` facility flag and OR-aggregation added by P1.105 (no schema/hierarchy changes). Corporate short-term gate is `original_maturity_years ≤ 0.25` only — Art. 121(4)/(5) trade-LC extension is institution-only. SME corporates excluded so the dedicated 85% SME path remains authoritative. CRR corporate branch unchanged. Pinned by `tests/acceptance/basel31/test_p1_103_art_122_3_short_term_corporate_ecai.py` (5 tests).
...
- **P1.128** [x] **FIXED v0.2.13** — PRA PS1/26 Art. 121(4) SCRA short-term trade-finance extension wired into the B31 institution chain. The B31 SCRA short-term gate in `_b31_append_institution_maturity_branches` (`engine/sa/namespace.py`) was hard-coded to `original_mty ≤ 0.25`, ignoring the `is_short_term_trade_lc & original_mty ≤ 0.5` OR-clause that the ECRA branch already had via the shared `in_st_window` expression. Replaced with `is_institution & is_unrated & in_st_window`. Also extended `engine/hierarchy.py::_propagate_facility_qrre_columns` with a counterparty-level OR-broadcast of `is_short_term_trade_lc` (mirroring the `has_short_term_ecai` precedent) so the flag propagates from facilities onto drawn-loan exposure rows when `facility_mappings` is empty. CRR helper unchanged. Pinned by `tests/acceptance/basel31/test_p1_128_art_121_4_scra_short_term_trade_finance.py` (5 tests).
...
- **P1.186** [x] **FIXED v0.2.13** — CRR Art. 224(2)(a) FX H_fx 20-day secured-lending default pinned by acceptance test. The engine fix shipped in commit `44006da` (2026-05-03) wired `exposure_is_sft` into the liquidation-period derivation in `engine/crm/haircuts.py`; this batch adds the missing acceptance test (10 tests covering: 20-day default for `is_sft=False` secured lending → ead_final ≈ 484,852.81; 5-day default for `is_sft=True` → ead_final ≈ 442,426.41; regression guard against the pre-fix 460,000.00 value; directional sanity SL > SFT). Closes the residual `liquidation_period as config` sub-item of P6.15. Pinned by `tests/acceptance/crr/test_p1_186_art_224_2_fx_haircut_secured_lending_default.py` (10 tests).
