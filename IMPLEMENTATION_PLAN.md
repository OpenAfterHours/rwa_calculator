...
Current state: 71/74 CRR tests pass (96%), 3 skipped. Additionally, CRR-B (F-IRB) has zero acceptance tests despite workbook scenarios existing — this is a significant uncounted gap.

- [ ] **CRR-B: F-IRB acceptance tests** — Workbook scenarios B1–B7 exist (`workbooks/crr_expected_outputs/scenarios/group_crr_b_firb.py`) covering corporate F-IRB low/high PD, subordinated, financial collateral LGD, SME F-IRB, PD floor binding, and maturity cap. However: (a) expected outputs in `tests/expected_outputs/crr/expected_rwa_crr.json` show `"CRR-B": {"count": 0}` — zero rows, (b) no test file `tests/acceptance/crr/test_scenario_crr_b_firb.py` exists. **Action:** Generate expected outputs from workbook, create acceptance test file with B1–B7 scenarios. See `specs/crr/firb-calculation.md`.
...
- CRR-B (F-IRB) is a previously uncounted gap — workbook scenarios exist but no acceptance tests or expected outputs were ever generated.
