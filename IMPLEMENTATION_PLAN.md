...
- **P1.97** [x] **FIXED v0.2.6** — Added `B31_SLOTTING_RISK_WEIGHTS_SHORT` (Strong=50%, Good=70%) and wired `is_short_maturity` through B31 `lookup_rw` for non-HVCRE per Art. 153(5)(d) Table A columns A/C. HVCRE column A/C still deferred to P1.117. `data/tables/b31_slotting.py` + `engine/slotting/namespace.py`.
- **P1.98** [x] **FIXED v0.2.6** — `_lgd_floor_expression` and `_lgd_floor_expression_with_collateral` no-`exposure_class` fallbacks now return `floors.unsecured` (25%) unconditionally per Art. 161(5); seniority sub-branch removed. Three pre-existing tests pinning the buggy 50% behaviour updated to assert 25%. The unused `LGDFloors.subordinated_unsecured = 0.50` constant remains in `contracts/config.py` pending separate cleanup.
...
- **P1.106** [x] **FIXED v0.2.6** — `_derive_collateral_rw_expr` institution branch now keys on `INSTITUTION_RISK_WEIGHTS_B31_ECRA` (CQS 2 → 30%) under B31 and `INSTITUTION_RISK_WEIGHTS_CRR` (CQS 2 → 50%) under CRR. `engine/crm/simple_method.py`.
- **P1.107** [x] **FIXED v0.2.7** — FCSM corporate CQS 3 collateral RW now reads `B31_CORPORATE_RISK_WEIGHTS[3]` (= 0.75) under B31 via new `corp_cqs3_rw` parametrisation in `engine/crm/simple_method.py::_derive_collateral_rw_expr`, mirroring the existing `corp_cqs5_rw` pattern. CRR retains 1.00 (regression-pinned). New acceptance suite at `tests/acceptance/basel31/test_p1_107_fcsm_b31_corporate_cqs3.py` (8 tests, including CRR control + framework-divergence delta). Ref: PRA PS1/26 Art. 122(2) Table 6.
...
- **P1.112** [x] **FIXED v0.2.6** — Replaced `cp_country_code == "GB"` hardcoded fallback with `PSE_RISK_WEIGHTS_SOVEREIGN_DERIVED` / `RGLA_RISK_WEIGHTS_SOVEREIGN_DERIVED` lookup keyed on `cp_sovereign_cqs` (Art. 116(1) Table 2 / Art. 115(1)(a) Table 1A). Applied to both CRR and B31 PSE/RGLA unrated branches via new `_sovereign_derived_rw_expr` helper; UK 0%/20% carve-outs preserved. Pre-existing UK PSE/RGLA tests now pin `sovereign_cqs=1` (the regulatorily correct UK value). `engine/sa/namespace.py`.
...
- **P1.117** [x] **FIXED v0.2.7** — Added `B31_SLOTTING_RISK_WEIGHTS_HVCRE_SHORT` (Strong=0.70, Good=0.95, Satisfactory=1.40, Weak=2.50, Default=0.00) in `data/tables/b31_slotting.py` per PRA PS1/26 Art. 153(5)(d) Table A HVCRE col A/C; wired into `engine/slotting/namespace.py` B31 `lookup_rw` so `is_hvcre & is_short` fires before the generic HVCRE catch-all. EL row unchanged (Table B HVCRE flat 0.4%). New acceptance suite at `tests/acceptance/basel31/test_p1_117_b31_hvcre_slotting_short.py` (13 tests).
...
- **P1.124** [x] **FIXED v0.2.7** — CRR Art. 237(2)(a) guarantee maturity ineligibility now enforced. Added `original_maturity_years` to `GUARANTEE_SCHEMA` (`data/schemas.py`) and an eligibility gate at the top of `engine/crm/guarantees.py::apply_guarantees` that filters out guarantee rows where `original_maturity_years.fill_null(10.0) < 1.0` (mirroring the collateral pattern at `engine/crm/haircuts.py:478-516`). New acceptance suite `tests/acceptance/crr/test_p1_124_art_237_2a_guarantee_ineligibility.py` (4 tests). Note: Art. 237(2)(a) residual-maturity prong (< 3 months) deferred — needs reporting_date plumbed onto guarantee rows; tracked separately. Ref: CRR Art. 237(2)(a).
- **P1.125** [x] **FIXED v0.2.7** — Added `ERROR_FSE_COLUMN_MISSING = "CLS007"` in `contracts/errors.py` and B31-only emission in `engine/classifier.py::ExposureClassifier.classify` mirroring the CLS005 retail-pool-management warning pattern. Fires when `is_financial_sector_entity` is absent from the source counterparty schema. New unit suite `tests/unit/classifier/test_p1_125_fse_column_warning.py` (8 tests covering scenarios A/B/C). Silent `_is_fse=lit(False)` default unchanged per warning-only brief. Ref: PRA PS1/26 Art. 147A(1)(e).
- **P1.126** [x] **FIXED v0.2.7** — Classifier null-revenue default is now conservative-large under B31. `_is_large_corp` predicate (`engine/classifier.py`) replaced `.fill_null(False)` with `pl.when(cp_annual_revenue.is_null()).then(True).otherwise(> 440m)` so unknown-revenue corporates are routed to F-IRB rather than silently granted A-IRB. Added new error code `ERROR_LARGE_CORP_REVENUE_NULL = "CLS008"` in `contracts/errors.py` and B31-gated emission in `ExposureClassifier.classify()` (mirrors the CLS007 P1.125 pattern). New unit suite `tests/unit/classifier/test_p1_126_null_revenue_warning.py` (11 tests covering scenarios A/B/C). Ref: PRA PS1/26 Art. 147A(1)(d).
...
- **P1.156** [x] **FIXED v0.2.7** — PSM guarantor LGD now seniority/FSE-aware. Replaced the hard-coded `firb_lgd_table["unsecured_senior"]` scalar in both `engine/irb/guarantee.py::_apply_parameter_substitution` and `_adjust_expected_loss` with a per-row Polars expression that routes by `guarantor_seniority == "subordinated"` (0.75) → `guarantor_is_financial_sector_entity` (0.45 FSE B31) → default senior unsecured (0.40 B31 / 0.45 CRR). Added `guarantor_seniority` column to `GUARANTEE_SCHEMA`. Widened `engine/irb/formulas.py::_parametric_irb_risk_weight_expr` `lgd` parameter to `float | pl.Expr` to accept the per-row expression (non-breaking). New unit suite `tests/unit/irb/test_p1_156_psm_guarantor_lgd_seniority.py` (6 parametrized cases across all three Art. 161 branches). Re-uses existing `BASEL31_FIRB_SUPERVISORY_LGD` keys; no data-table changes. Ref: CRR Art. 236, PRA PS1/26 Art. 161(1)(a)/(aa)/(b).
...
- **P1.182** [x] **FIXED v0.2.7** — PE/VC routing now applies the PRA PS1/26 Glossary p.5 two-condition test (unlisted AND business_age_years < 5). Added `business_age_years: ColumnSpec(pl.Float64, required=False)` to `EQUITY_EXPOSURE_SCHEMA` and `_EQUITY_INPUT_CONTRACT`. Replaced the unconditional `private_equity` / `private_equity_diversified` → 400% mapping in `engine/equity/calculator.py::_apply_b31_equity_weights_sa` with `is_higher_risk_pe = is_pe_or_pe_div AND is_unlisted AND (business_age_years < 5.0 OR null)` — long-established PE falls through to OTHER (250%, Art. 133(3)). Same predicate added to `_apply_transitional_floor::is_hr_pe` so the 2030 higher-risk floor doesn't clobber the corrected weight. Both predicates guard via `collect_schema().names()` for backwards compat with fixtures missing the column. New acceptance suite `tests/acceptance/basel31/test_p1_182_b31_l24_pe_unlisted_under_5y.py` (6 tests across long-established / young / null-age scenarios). Data tables unchanged. Ref: PRA PS1/26 Glossary p.5, Art. 133(3)-(4).
...
- **P1.187** [x] **FIXED v0.2.7** — FCSM CRR CQS 5 corporate-bond collateral RW corrected from 100% to 150% per Art. 122 Table 5. The engine literal `1.00` at `engine/crm/simple_method.py::_derive_collateral_rw_expr` (corporate branch) understated capital relative to the regulatorily-correct 150%; the wrongly-pinned `test_corporate_cqs5_crr_hundred_pct` regression test masked the issue. Fixed inline as part of a wider refactor that wires the function's sovereign / corporate / equity branches to `CENTRAL_GOVT_CENTRAL_BANK_RISK_WEIGHTS`, `CORPORATE_RISK_WEIGHTS` (CRR), `B31_CORPORATE_RISK_WEIGHTS` (B31) and a new `FCSM_EQUITY_COLLATERAL_RW` constant in `data/tables/crr_simple_method.py`. Test renamed to `test_corporate_cqs5_crr_one_fifty_pct`. **Capital impact**: small (only CQS 5 CRR corporate bond collateral exposures via FCSM); previously understated by 50 percentage points on the FCSM-substituted slice. **Note**: function-local literals escape `arch_check.py` checks 5/6 (module-scope only) — extending the AST walk to function bodies remains open as a Tier 6 follow-up. **Effort: S** | Ref: CRR Art. 222(1), Art. 122 Table 5.
...
- **P1.144** [x] **FIXED v0.2.7** — `IRBCalculator.calculate_expected_loss` now unconditionally materialises `ead_final` (aliasing `pl.col("ead")` when absent, mirroring `engine/irb/namespace.py::prepare_columns:236-240`); EL formula and output `.select(...)` projection both pinned to `ead_final`. Eliminates the divergence between EL (formerly fell back to gross `ead`) and the downstream `_el_summary` consumer (always reads `ead_final`). New unit suite `tests/unit/irb/test_p1_144_el_ead_final_consistency.py` (4 tests). Ref: CRR Art. 158, Art. 159.
...
- **Status:** [x] **FIXED v0.2.7 (batch 20260503-2159) — gate widened + acceptance test added**
- **Verification:** `INSTITUTION_SHORT_TERM_RISK_WEIGHTS_CRR` exists at `data/tables/crr_risk_weights.py:140-147` with CQS1-3 = 20%, CQS4-5 = 50%, CQS6 = 150% per Art. 120(2) Table 4. The CRR SA chain at `engine/sa/namespace.py::_crr_append_institution_maturity_branches` now fires on `(residual_mty <= 0.25) | (original_mty <= 0.25)` (was residual-only — date-derived original maturity wasn't reaching the gate). Matches the existing B31 ECRA pattern. New acceptance suite `tests/acceptance/crr/test_p1_99_crr_institution_short_term.py` (24 tests across CQS 1-6).
...
- **Status:** [x] **FIXED v0.2.7 (batch 20260503-2248) — regression test added (silent fix from prior audit)**
- **Verification:** `INSTITUTION_SHORT_TERM_UNRATED_RW_CRR = Decimal("0.20")` at `data/tables/crr_risk_weights.py:151` and applied at `engine/sa/namespace.py:_crr_append_institution_maturity_branches` via `chain.when(is_institution & is_unrated & (original_mty <= 0.25)).then(...)`. New regression-only acceptance suite `tests/acceptance/crr/test_p1_121_art_121_3_unrated_inst_short_term.py` (5 tests, all PASS — pin RW=0.20, RWA=200,000 for an institution VN sovereign_cqs=4 USD 1m drawn with 85d original maturity).
