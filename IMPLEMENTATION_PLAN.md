...
- **P8.1** [ ] **NEW — first-batch (1/16)** — Trade-level input bundles. Add `@dataclass(frozen=True)` `TradeBundle`, `NettingSetBundle`, `MarginAgreementBundle`, `CCRCollateralBundle` to `contracts/bundles.py`. Trade attributes: `trade_id`, `netting_set_id`, `asset_class` (IR/FX/credit/equity/commodity/other), `transaction_type` (option, swap, forward, future), `notional`, `currency`, `maturity_date`, `start_date`, `delta`, `is_long`, `mtm_value`, optional `underlying_reference`, `option_strike`, `payment_leg_index_id`. Netting-set attributes: `netting_set_id`, `is_legally_enforceable` Bool, `margin_type` (`unmargined` / `margined_one_way` / `margined_two_way`), `threshold`, `mta`, `nica`, `mpor_days`. Mirror `CRMAdjustedBundle` pattern (`contracts/bundles.py:188-229`). **Effort: M** | Ref: CRR Art. 272 definitions, Art. 295-297 netting recognition
- **P8.2** [ ] **NEW — first-batch (2/16)** — Extend `RawDataBundle` with optional `ccr: RawCCRBundle | None` field. `RawCCRBundle` aggregates trade / netting-set / margin / collateral frames. Backward-compatible (None → CCR stage no-ops). Update `contracts/bundles.py` (current `RawDataBundle` at lines 36-84). **Effort: S**
- **P8.3** [ ] **NEW — first-batch (3/16)** — `CCRCalculator` Protocol in `contracts/protocols.py`. Methods: `compute_ead(trades: LazyFrame, netting_sets: LazyFrame, margin: LazyFrame, collateral: LazyFrame, config: CCRConfig) -> LazyFrame` returning rows keyed by `netting_set_id` with `ead_ccr`, `rc`, `pfe`, `alpha`, `ccr_method` columns. Mirror `CRMProcessorProtocol` (protocols.py:187-259). **Effort: S**
- **P8.5** [ ] **NEW — first-batch (4/16)** — Loader extension + schemas. New schemas in `data/schemas.py` (mirror `FACILITY_SCHEMA` at lines 60-117): `TRADE_SCHEMA`, `NETTING_SET_SCHEMA`, `MARGIN_AGREEMENT_SCHEMA`, `CCR_COLLATERAL_SCHEMA`. Schemas must extend `risk_type` constraint set to accept `CCR_DERIVATIVE` and `CCR_SFT`. `engine/loader.py` learns to read `trades.parquet`, `netting_sets.parquet`, `margin_agreements.parquet`, `ccr_collateral.parquet`. **Effort: M**
- **P8.6** [ ] **NEW — first-batch (5/16)** — `CCRConfig` dataclass added to `contracts/config.py`. Fields: `method: Literal["sa_ccr"]` (Simplified/OEM deferred per scope decision 1), `alpha: Decimal = Decimal("1.4")`, `enable_ccp_exposures: bool = True`, `mpor_floor_days: int = 10`, `recognise_im: bool = True`. Wire into `CalculationConfig.crr()` (config.py:952-1009) and `.basel_3_1()` (config.py:1011-1101) factories with regulator-appropriate defaults. **Effort: S** | Ref: CRR Art. 274(2) α
- **P8.7** [ ] **NEW — first-batch (6/16)** — `data/tables/sa_ccr_factors.py` — pure-data tables for SA-CCR. Supervisory factors per asset class & sub-class (Art. 280 Annex / CRR Annex II): interest-rate by currency bucket, FX, credit single-name by IG/HY/non-rated, credit index by IG/HY, equity single-name vs index, commodity by sub-type. Supervisory correlations (Art. 280 (1)-(3)). Maturity-factor constants. Mirror `crr_risk_weights.py` pattern (`Decimal` dicts + `_build_*_df()` helper). Must satisfy `arch_check.py` checks 5/6 (no regulatory scalars in `engine/**`). **Effort: M** | Ref: CRR Art. 280, Annex II; PRA PS1/26 Title II Chapter 6 §6
