...
- **P1.177** [ ] **NEW** — CRR HVCRE slotting code bug: `crr_slotting.py` defines `SLOTTING_RISK_WEIGHTS_HVCRE`/`_SHORT` from EU Table 2; UK CRR has no HVCRE concept (Art. 153(5) Table 1 covers all SL). **Capital impact: overstatement** for `is_hvcre=True` CRR exposures. Route through non-HVCRE Table 1 under CRR; update CRR-E4/E7/E8. **Effort: S** | Ref: CRR Art. 153(5)
...
- **P1.181** [ ] **NEW** — CRR Art. 126 CRE is binary whole-loan; should be proportion split. `crr_risk_weights.py::calculate_commercial_re_rw` + `calculator.py:984-997` apply 50% (LTV ≤ 50% + income cover) or 100%. Art. 126(2)(d) requires 50% on portion ≤ 50% MV, counterparty RW on excess (mirror Art. 125 RRE split). **Capital impact: overstatement** for CRE LTV > 50%. **Effort: S** | Ref: UK CRR Art. 126(2)(d)
