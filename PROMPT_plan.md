...
ULTIMATE GOAL: Achieve 100% CRR (Basel 3.0) acceptance test pass rate, then complete Basel 3.1 (PRA PS9/24) implementation with full acceptance test coverage across both frameworks. Consider missing elements and plan accordingly. If an element is missing, search first to confirm it doesn't exist, then if needed author the specification at `specs/`. If you create a new element then document the plan to implement it in @IMPLEMENTATION_PLAN.md using a subagent.
