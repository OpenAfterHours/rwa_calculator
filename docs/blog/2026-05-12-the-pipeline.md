# The Pipeline: Why Regulation Forced an Immutable Design
...
*Update (June 2026): several concrete code references below were overtaken by the 2026-06 architecture migration. The principles (frozen bundles, structural protocols, error accumulation, a script-enforced data/engine split) all still hold, but: (1) the pipeline is no longer a hand-wired chain of eight bundle-returning stages — it is now a fold over a literal `StageSpec` registry (`engine/registry.py` + `engine/orchestrator.py::run_stages`) threading an immutable typed `PipelineContext`, with `engine/pipeline.py` reduced to a run-lifecycle facade, and the registry has since grown from the eight stages above to ten with the addition of `securitisation_allocator`, `ccr_sa_ccr`, and `sft_fccm`; (2) cross-stage laziness was replaced by eagerly-sealed edge frames (`materialise_barrier` was renamed `materialise_edge`; `config.collect_engine` still exists and defaults to `'cpu'`, while a new opt-in `config.spill_edges` flag — default `False` — selects the spill-to-parquet path, and only the deprecated `config.collect_engine='streaming'` spelling was superseded by it); (3) `RegulatoryThresholds` and the other regulatory sub-configs were removed from `CalculationConfig` — regulatory thresholds and scalars now live in the rulebook packs (`src/rwa_calc/rulebook/packs/{common,crr,b31}.py`) as cited entries, resolved per regime + date via `rwa_calc.rulebook.resolve`, so `with_fx_rate()` no longer rebuilds a thresholds object and the value home moved from `data/tables/` to the packs (the "read one file" property is now satisfied by the relevant pack). Separately, `contracts/bundles.py` has roughly doubled (now ~1,217 lines, ~19 bundle classes including the CCR, SFT, and comparison/analysis bundles) as those subsystems landed; the pinned line counts below are historical. See the [Architecture: Pipeline](../architecture/pipeline.md) page for the current model.*
