...
**Source**: `RAW_EXPOSURE_SCHEMA` in `data/schemas.py`, `HierarchyResolver._unify_exposures()` in `engine/hierarchy/resolver.py`
...
**Source**: `RESOLVED_HIERARCHY_SCHEMA` in `data/schemas.py`, `HierarchyResolver.resolve()` in `engine/hierarchy/resolver.py`
...
**Source**: `CLASSIFIED_EXPOSURE_SCHEMA` in `data/schemas.py`, `ExposureClassifier.classify()` in `engine/classify/classifier.py`
