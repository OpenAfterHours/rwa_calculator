...
**Source**: `RAW_EXPOSURE_SCHEMA` in `data/schemas.py`, `HierarchyResolver._unify_exposures()` in `engine/stages/hierarchy/resolver.py`
...
**Source**: `RESOLVED_HIERARCHY_SCHEMA` in `data/schemas.py`, `HierarchyResolver.resolve()` in `engine/stages/hierarchy/resolver.py`
...
**Source**: `CLASSIFIED_EXPOSURE_SCHEMA` in `data/schemas.py`, `ExposureClassifier.classify()` in `engine/stages/classify/classifier.py`
