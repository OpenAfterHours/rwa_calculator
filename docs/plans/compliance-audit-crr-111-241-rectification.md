...
- **P1.238** [🟠 MED] · **CRR Art. 195; PS1/26 Art. 195** (both) · effort M
  ...
  - _Files:_ `collateral.py`
  - _2026-09-04 — REVERSED by operator decision:_ the netting perimeter is the agreement reference (Feature `on_bs_netting_perimeter_is_agreement`, enabled both regimes); the single-counterparty keying this finding introduced is retained behind the Feature (`enabled=False`). Basis: the agreement is the set-off boundary; joint-and-several parties are the Art. 195 counterparty; Art. 205(a) enforceability is the firm's attestation. CRM016 survives as an audit-trail warning rather than a refusal. Documented at `docs/user-guide/methodology/crm.md` — "The Netting Perimeter Is the Agreement".
