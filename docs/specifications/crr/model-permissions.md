...
- Per-exposure `model_id` lookup against the `model_permissions` data source happens
  in [`engine/classify/permissions.py`](https://github.com/OpenAfterHours/rwa_calculator/blob/master/src/rwa_calc/engine/classify/permissions.py)
  (imported from the `engine/classify/` package)
