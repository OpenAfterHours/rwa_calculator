...
class ReconciliationResponse:
    ...
    Attributes:
        ...
        legacy_ledger: The legacy extract projected into the sealed
            reporting-ledger vocabulary — a ``ResultsSource``, so the firm's side
            of a return is generated by the same COREP generators as ours. Lazy:
            nothing is collected to build it and no template is generated here.
            None when the projection could not be produced (it raised, or the
            mapping reached no scoped template); the reason is then on ``errors``
            as a non-blocking REC008 warning.
        legacy_ledger_coverage: What that mapping could and could not supply —
            which templates are reachable at all, and which cells cannot be
            populated. Present exactly when ``legacy_ledger`` is.
    """
    ...
    # Optional by construction: every pre-existing construction site predates the
    # projection and must keep working untouched.
    legacy_ledger: LegacyLedgerSource | None = None
    legacy_ledger_coverage: LedgerCoverage | None = None
    ...
    def from_bundle(
        ...
        *,
        ...
        return cls(
            ...
            legacy_ledger_coverage=legacy_ledger_coverage,
        )
