"""
Shared helpers for result aggregation.

Provides constants, utility functions, and extracted computation logic used by
both ``OutputAggregator`` (class-based API) and ``AggregatorLazyFrame``
(Polars namespace API). Internal to the engine package.

Pipeline position:
    SACalculator/IRBCalculator/SlottingCalculator -> helpers -> Pipeline output
...
# --- Post-CRM detailed sub-functions ---
