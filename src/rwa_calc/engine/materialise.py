...
The pipeline has several points where lazy plans must be materialized to keep
the native plan tree shallow. The constraint is recursive plan-tree DEPTH, not
executor capacity: on very deep plans Polars hard-crashes (SIGSEGV) during
plan construction, the optimizer pass inside ``collect()``, or Rust ``Drop``
teardown of the nested plan nodes — all BEFORE any executor runs, so the
streaming engine does not avoid it. Measured on Polars 1.37: the crash
threshold is ~25,000 plan nodes for trivial ``with_columns`` chains and far
lower for heavy ``when/then`` + join expressions. The barriers also bound
plan-construction TIME — without them, plan construction and optimizer passes
re-walk the full upstream per consumer (~100x slowdown measured on a 150-row
fixture). The node threshold is a property of the installed Polars version and
must be re-measured on every Polars upgrade. Full investigation:
docs/plans/single-lazy-plan-refactor.md.
