"""
FX conversion kernel for the FX stage package.

Pipeline position:
    HierarchyResolver (unify) -> FX conversion -> HierarchyResolver (enrich)

Key responsibilities:
- ``FXConverter``: stateless five-method conversion kernel — exposures,
  collateral, guarantees, provisions, equity exposures — rebasing amounts
  to ``config.base_currency`` with ``original_currency`` /
  ``original_amount`` / ``fx_rate_applied`` audit columns.
- Null-currency and missing-rate rows pass through unconverted (audit
  columns still added); when conversion is disabled the audit columns are
  added without rebasing.
- ``create_fx_converter``: factory.

References:
- CRR Art. 224 / Art. 233(3)-(4): downstream FX-mismatch haircuts read the
  ``original_currency`` audit column this kernel preserves
- docs/plans/target-architecture-migration.md (Phase 4)

Usage:
    from rwa_calc.engine.stages.fx import FXConverter
