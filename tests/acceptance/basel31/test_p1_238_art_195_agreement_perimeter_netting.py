"""
Basel 3.1 — the on-B/S netting perimeter is the netting AGREEMENT, not the counterparty.
...
agreement AGR1. Only the loan's counterparty differs:
    ...
    b31_cross_cp — loan owed by a DIFFERENT counterparty
...
by operator decision dated 2026-09-04. The regime twin of the CRR file exists
because a perimeter change is regime-gated by a pack Feature: a both-regimes
claim has to be shown to fail in each regime separately (LESSONS C7), so this
file must be able to go red on its own.
...
References:
    - PS1/26 Art. 195: on-B/S netting of reciprocal balances.
    - PS1/26 Art. 205(a): enforceability against all parties.
    - PS1/26 Art. 219: drawn-on-drawn cash netting.
    ...
    disabled = RulepackV0.from_config(config).pack.with_overrides(
        **{
            NETTING_PERIMETER_FEATURE: Feature(
                ...
                enabled=False,
                citation=Citation("PS1/26", "195", "agreement perimeter disabled for test"),
            )
        }
    )
    bundle = build_p1_238_bundle([scenario_label], REPORTING_DATE)
...
def _assert_scenarios_are_adequate() -> None:
    ...
    same = SCENARIOS["b31_same_cp"]
    cross = SCENARIOS["b31_cross_cp"]
...
class TestB31AgreementPerimeterNetting:
    """Basel 3.1: the agreement reference alone bounds on-B/S netting."""
    ...
    def test_same_counterparty_loan_nets(self) -> None:
        ...
        # Assert
        assert _loan_rwa(result, SCENARIOS["b31_same_cp"].loan_ref) == pytest.approx(
    ...
    def test_cross_counterparty_loan_nets_under_shared_agreement(self) -> None:
        ...
        # Assert
        assert _loan_rwa(result, SCENARIOS["b31_cross_cp"].loan_ref) == pytest.approx(
            RWA_SAME_CP_NETTED, rel=1e-3
        )
    ...
    def test_cross_counterparty_emits_crm016_audit_record(self) -> None:
        ...
        """
        # Arrange
        scenario = SCENARIOS["b31_cross_cp"]
    ...
    def test_feature_disabled_restores_no_cross_counterparty_netting(self) -> None:
        ...
        # Act
        disabled = _loan_rwa(
            _run_perimeter_disabled("b31_cross_cp"), SCENARIOS["b31_cross_cp"].loan_ref
        )
        default = _loan_rwa(_run("b31_cross_cp"), SCENARIOS["b31_cross_cp"].loan_ref)
    ...
    def test_feature_disabled_leaves_same_counterparty_netting_intact(self) -> None:
        ...
        # Act
        result = _run_perimeter_disabled("b31_same_cp")
