"""
P1.275 Basel 3.1 — Art. 232(3)/233(3): life-insurance FX reduction + multi-level pledge.

The Basel 3.1 twin of the CRR P1.275 scenarios. Art. 232(3) (secured-portion RW
map) and Art. 233(3) (8% FX volatility haircut) are retained unchanged under
PS1/26, so the expected RWA is identical to the CRR case:
...
References:
    - PS1/26 Art. 232(3)/233(3): life-insurance funded protection (retained).
...
def _scenario_rwa(result, scenario_label: str) -> float:
    ...
    for loan in SCENARIOS[scenario_label].loans:
        ...
        assert rows, f"P1.275 B31: no result rows for {loan.ref}"
...
class TestP1275B31LifeInsurance:
    """Basel 3.1: Art. 232(3)/233(3) retained -> identical FX reduction and pro-rata pledge."""
