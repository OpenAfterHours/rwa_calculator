"""
P1.314 (Basel 3.1 / PS1/26) — Art. 114(7): the funding limb must be tested
against the **pre-FX denomination**, not the post-FX reporting currency.

Pipeline position:
    Loader -> HierarchyResolver -> Classifier -> FXConverter -> CRMProcessor
    -> SACalculator -> Aggregator

This is the Basel 3.1 half of a deliberately regime-split pair. The two
override chains — ``risk_weights.py::_apply_b31_risk_weight_overrides`` and
``_apply_crr_risk_weight_overrides`` — are separate functions that share no
code, yet every expected value is IDENTICAL across regimes because both read
the same ``cgcb_risk_weights`` pack entry. A single red observed across a
collapsed both-regimes run would prove one chain carries the limb, not two
(.claude/LESSONS.md C7), so the CRR sibling
(``tests/acceptance/crr/test_p1_314_art_114_7_funded_currency.py``) is a
separate file and both must be seen to fail independently.

CRR Art. 114(7) survives unchanged under Basel 3.1: PS1/26 Art. 114(1) lists
its own carve-outs as "(a) paragraphs 2 to 4 and Article 137(2); (b) **Article
114(7) of CRR**; ...", so the third-country/EU domestic-currency extension is
live in this regime by cross-reference rather than by restatement.

Why no unit leg can replace this file: ``calculate_single_sa_exposure`` builds
no ``original_currency`` column, so ``denomination_currency_expr`` collapses to
``pl.col("currency")`` there and an implementation comparing the funding
currency against the post-FX ``currency`` is indistinguishable from the correct
one on every unit leg. Measured on a real post-FX frame:

    correct             FUNDEUR rw=0.00   FUNDUSD rw=1.50
    wrong currency col  FUNDEUR rw=1.50   FUNDUSD rw=1.50

References:
    - PS1/26 Art. 114(1)(b) -> CRR Art. 114(7): "denominated and funded in the
      domestic currency".
...
def _row(df: pl.DataFrame, loan_ref: str) -> dict:
    ...
    assert len(rows) == 1, (
        f"P1.314 B31: expected exactly 1 result row for {loan_ref}, got {len(rows)}. "
...
def sa_results() -> pl.DataFrame:
    """One Basel 3.1 run of the P1.314 bundle; the SA branch must have been emitted."""
    results = PipelineOrchestrator().run_with_data(build_p314_bundle(), _config())
    assert results.sa_results is not None, (
        "P1.314 B31: sa_results is None under STANDARDISED — the SA branch was "
        "not emitted at all, so no risk-weight assertion below means anything."
    )
    return results.sa_results.collect()


class TestP1314Art1147FundedCurrencyB31EndToEnd:
    """Basel 3.1: the Art. 114(7) funding limb across a real FX-converted frame."""

    # ---- PRECONDITION: the frame really is post-FX --------------------------

    def test_p1_314_b31_rows_are_post_fx_eur_denominated(self, sa_results: pl.DataFrame) -> None:
        ...
        for row in rows:
            ...
            assert row["currency"] == "GBP", (
                f"P1.314 B31: {ref} should report in GBP after FX conversion, got "
    ...
    def test_p1_314_b31_usd_funded_eu_sovereign_loses_the_zero_extension(
        ...
        # Assert
        assert row["risk_weight"] is not None, "P1.314 B31: risk_weight is NULL on the mover."
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_FUNDUSD, abs=1e-9), (
            f"P1.314 B31: {LOAN_FUNDUSD_REF} risk_weight should be "
            ...
            f"reached the direct exposure path in the Basel 3.1 override chain."
        )
        assert row["rwa_final"] is not None, "P1.314 B31: rwa_final is NULL on the mover."
        assert row["rwa_final"] == pytest.approx(
            EXPECTED_EAD_GBP * EXPECTED_RW_FUNDUSD, abs=1e-9
        ), (
            f"P1.314 B31: {LOAN_FUNDUSD_REF} rwa_final should be "
            f"{EXPECTED_EAD_GBP * EXPECTED_RW_FUNDUSD:,.2f}, got {row['rwa_final']:,.4f}."
        )

    # ---- DISCRIMINATING against the wrong-column implementation -------------

    def test_p1_314_b31_eur_funded_eu_sovereign_keeps_the_zero_extension(
        self, sa_results: pl.DataFrame
    ) -> None:
        ...
        # Assert
        assert row["risk_weight"] is not None, "P1.314 B31: risk_weight is NULL on the survivor."
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_FUNDEUR, abs=1e-9), (
            f"P1.314 B31: {LOAN_FUNDEUR_REF} risk_weight should be "
        ...
        assert row["rwa_final"] is not None, "P1.314 B31: rwa_final is NULL on the survivor."
        assert row["rwa_final"] == pytest.approx(0.0, abs=1e-9), (
            f"P1.314 B31: {LOAN_FUNDEUR_REF} rwa_final should be 0.00, got {row['rwa_final']:,.4f}."
        )

    # ---- NEGATIVE SPACE -----------------------------------------------------

    def test_p1_314_b31_branch_reason_is_populated_on_both_rows(
        self, sa_results: pl.DataFrame
    ) -> None:
        ...
        for row in rows:
            ...
            assert reason is not None, (
                f"P1.314 B31: {row['source_exposure_reference']} carries a NULL "
                f"{SA_RISK_WEIGHT_BRANCH_REASON}."
            )
            assert reason != UNKNOWN_FALLBACK, (
                f"P1.314 B31: {row['source_exposure_reference']} is named "
                f"{UNKNOWN_FALLBACK} — a domesticity predicate went indeterminate."
            )

    def test_p1_314_b31_funding_currency_raises_no_new_error_codes(self) -> None:
        ...
        control run rather than as "no errors", because the Basel 3.1 baseline
        is legitimately non-empty (SA004, due diligence).
        """
        # Arrange / Act
        ...
        assert live_codes == control_codes, (
            f"P1.314 B31: reporting a funding currency changed the error channel. "
