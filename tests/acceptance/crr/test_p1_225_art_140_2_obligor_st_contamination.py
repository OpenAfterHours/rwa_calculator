"""
P1.225: CRR Art. 140(2) obligor-level short-term rating contamination.

Pipeline position:
    Loader -> HierarchyResolver -> Classifier -> CRMProcessor -> SACalculator -> Aggregator

Key responsibilities:
- Confirm that when an obligor's short-term (ST) rated facility attracts
  150% (Table 7 CQS 4), Art. 140(2)(a) broadcasts 150% to ALL of that
  obligor's unrated UNSECURED exposures — short- OR long-term (E2).
- Confirm that when an obligor's ST rated facility attracts 50% (Table 7
  CQS 2), Art. 140(2)(b) floors ALL of that obligor's unrated ST unsecured
  exposures at 100% (E4), even when the class-default ST treatment (SCRA
  grade A, 20%) would otherwise be more favourable.
- Confirm the ST-rated triggers themselves (E1, E3) are unaffected (they
  ARE the contamination source, not a target).
- Confirm an isolated obligor with no ST-rated facility anywhere (E6, CP-Z)
  never triggers contamination.
- Confirm a GUARANTEED exposure (E5) is excluded from contamination — Art.
  140(2)(a) only reaches UNRATED UNSECURED exposures, and a guarantee
  substitution is not "unsecured".

Hand-calculation (CRR, CalculationConfig.crr(), Table 7 CQS-to-RW,
numerically identical to Basel 3.1 at these two bands — see
tests/fixtures/p1_225/p1_225.py for the full derivation):

    E1 (CP-X, ST-CQS4 trigger):        RW = 1.50 (Table 7)         -> RWA = 1,500,000
    E2 (CP-X, unrated, long-term):     pre-fix RW = 1.00 (class default)
                                        post-fix RW = 1.50 (Art. 140(2)(a) 150% broadcast)
    E3 (CP-Y, ST-CQS2 trigger):        RW = 0.50 (Table 7)         -> RWA =   500,000
...
References:
    - CRR Art. 140(2) (CRE21.17-21.18): obligor-level short-term-assessment
      contamination — 150% broadcast (a) / 100% floor (b).
...
# Reporting date: ~30 days after value_date (2027-01-01) and comfortably
# after the ratings' rating_date (2027-01-02), mirroring the p1_216
# convention. Residual to the ST maturity (2027-03-15) is 43 days ~= 0.118y,
# well within the 0.25y ST window; residual to the long-term maturity
...
def _build_bundle() -> RawDataBundle:
    """
    Build the P1.225 RawDataBundle: counterparty/loan/rating/guarantee
    parquets, no facilities (loan-scoped ratings, mirrors p1_216/p1_223).
    """
...
def sa_results() -> pl.DataFrame:
    """CRR SA pipeline results over the full P1.225 fixture set (one run)."""
...
# ---------------------------------------------------------------------------
# P1.225 acceptance tests — CRR
# ---------------------------------------------------------------------------


class TestP1225Art1402ObligorSTContaminationCRR:
    """P1.225: CRR Art. 140(2) obligor-level ST contamination — 150% broadcast / 100% floor."""
    ...
    def test_p1_225_e2_contaminated_to_150_pct(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E2_POST_FIX, abs=1e-4), (
            f"P1.225: expected E2 risk_weight={EXPECTED_RW_E2_POST_FIX:.2f} "
        ...
        assert row["rwa_final"] == pytest.approx(EXPECTED_RWA_E2_POST_FIX, rel=1e-4), (
            f"P1.225: expected E2 rwa_final={EXPECTED_RWA_E2_POST_FIX:,.0f} "
    ...
    def test_p1_225_e4_floored_at_100_pct(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E4_POST_FIX_FLOOR, abs=1e-4), (
            f"P1.225: expected E4 risk_weight={EXPECTED_RW_E4_POST_FIX_FLOOR:.2f} "
            f"(Art. 140(2)(b) 100% floor, max(20%, 100%)), got {row['risk_weight']:.4f}"
        )
        assert row["rwa_final"] == pytest.approx(1_000_000.0, rel=1e-4), (
            f"P1.225: expected E4 rwa_final=1,000,000 (EAD 1,000,000 x 100% floor), "
    ...
    def test_p1_225_e1_trigger_unchanged(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E1, abs=1e-4), (
            f"P1.225: expected E1 risk_weight={EXPECTED_RW_E1:.2f} (Table 7 CQS 4), "
            f"got {row['risk_weight']:.4f}"
        )
        assert row["rwa_final"] == pytest.approx(EXPECTED_RWA_E1, rel=1e-4), (
            f"P1.225: expected E1 rwa_final={EXPECTED_RWA_E1:,.0f}, got {row['rwa_final']:,.2f}"
        )
    ...
    def test_p1_225_e3_trigger_unchanged(self, sa_results: pl.DataFrame) -> None:
        ...
        contamination fix — its own rating already carries the Table 7
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E3, abs=1e-4), (
            f"P1.225: expected E3 risk_weight={EXPECTED_RW_E3:.2f} (Table 7 CQS 2), "
            f"got {row['risk_weight']:.4f}"
        )
        assert row["rwa_final"] == pytest.approx(EXPECTED_RWA_E3, rel=1e-4), (
            f"P1.225: expected E3 rwa_final={EXPECTED_RWA_E3:,.0f}, got {row['rwa_final']:,.2f}"
        )
    ...
    def test_p1_225_e6_isolated_control_unchanged(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E6_CONTROL, abs=1e-4), (
            f"P1.225: expected E6 risk_weight={EXPECTED_RW_E6_CONTROL:.2f} "
            f"(unrated corporate class default, no ST trigger on CP-Z), "
            f"got {row['risk_weight']:.4f}"
        )
        assert row["rwa_final"] == pytest.approx(EXPECTED_RWA_E6, rel=1e-4), (
            f"P1.225: expected E6 rwa_final={EXPECTED_RWA_E6:,.0f}, got {row['rwa_final']:,.2f}"
        )
    ...
    def test_p1_225_e5_guaranteed_leg_excluded_from_contamination(
        ...
        assert row["risk_weight"] == pytest.approx(EXPECTED_RW_E5_GUARANTOR, abs=1e-6), (
            f"P1.225: expected E5 guaranteed-leg risk_weight="
    ...
    def test_p1_225_e5_remainder_leg_ead_and_rwa_are_zero(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["ead_final"] == pytest.approx(0.0, abs=1.0), (
            f"P1.225: expected E5 remainder-leg ead_final=0, got {row['ead_final']:,.2f}"
        )
        assert row["rwa_final"] == pytest.approx(0.0, abs=1.0), (
            f"P1.225: expected E5 remainder-leg rwa_final=0 (ead_final=0 regardless "
            f"of risk_weight), got {row['rwa_final']:,.2f}"
        )
    ...
    def test_p1_225_portfolio_total_rwa_reflects_full_contamination(
        ...
        assert total_rwa == pytest.approx(_EXPECTED_TOTAL_RWA_POST_FIX, rel=1e-4), (
            f"P1.225: expected portfolio total rwa_final="
    ...
    def test_p1_225_e2_e4_do_not_carry_own_short_term_ecai_flag(
        ...
        is False on both E2 and E4 under CRR today. Should PASS today and
        ...
        assert e2["has_short_term_ecai"] is False, (
            f"P1.225: E2 has_short_term_ecai should be False (E2 has no issue-specific "
            f"ST rating of its own — it is contaminated via Art. 140(2), not "
            f"Art. 120(3)(c)), got {e2['has_short_term_ecai']!r}"
        )
        assert e4["has_short_term_ecai"] is False, (
            f"P1.225: E4 has_short_term_ecai should be False (E4 has no issue-specific "
            f"ST rating of its own — it is floored via Art. 140(2)(b), not "
            f"Art. 120(3)(c)), got {e4['has_short_term_ecai']!r}"
        )
