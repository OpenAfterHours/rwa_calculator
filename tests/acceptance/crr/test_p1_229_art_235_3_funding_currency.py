"""
P1.229 (CRR) — Art. 235(3): the domestic-CGCB 0% extension requires the exposure
to be BOTH denominated AND *funded* in the guarantor's domestic currency.

Pipeline position:
    Loader -> HierarchyResolver -> Classifier -> CRMProcessor -> SACalculator -> Aggregator

Key assertion:
    A EUR-denominated, EUR-guaranteed loan from a DE sovereign (external CQS 3)
    passes the denomination limb of Art. 114(4)/(7). The 0% extension then hinges
    on the funding currency:

        LN_P229_MISMATCH (funded USD): 0% extension DENIED -> guaranteed portion
            risk weight = 0.50 (DE sovereign CQS 3, cgcb_risk_weights[CQS3]).
            PRE-FIX BUG: engine ignores the funding limb -> 0.00.
        LN_P229_MATCHED  (funded EUR): 0% extension applies -> 0.00 (regression).
        LN_P229_NULL     (funding null): PERMISSIVE fallback to the EUR
            denomination -> 0.00 (regression; documents the null policy).

References:
    - CRR Art. 235(3) / Art. 114(4)/(7): denominated-and-funded 0% condition.
...
def _run_sa() -> pl.DataFrame:
    """Run the P1.229 fixture through the CRR SA pipeline; return SA results."""
...
class TestP1229Art2353FundingCurrencyCRR:
    """P1.229 CRR: the Art. 235(3) funding limb on the domestic-CGCB 0% extension."""
    ...
    def test_usd_funded_eur_guaranteed_gets_sovereign_50pct_not_zero(
        ...
        assert actual == pytest.approx(EXPECTED_RW_MISMATCH_POSTFIX, abs=1e-9), (
            f"P1.229 CRR: USD-funded EUR-guaranteed guaranteed-portion risk_weight "
    ...
    def test_eur_funded_eur_guaranteed_keeps_zero(self, sa_results: pl.DataFrame) -> None:
        ...
        assert actual == pytest.approx(EXPECTED_RW_MATCHED, abs=1e-9), (
            f"P1.229 CRR: EUR-funded EUR-guaranteed guaranteed-portion risk_weight "
    ...
    def test_null_funding_currency_is_permissive(self, sa_results: pl.DataFrame) -> None:
        ...
        assert actual == pytest.approx(EXPECTED_RW_NULL, abs=1e-9), (
            f"P1.229 CRR: null-funding guaranteed-portion risk_weight should be "
    ...
    def test_mismatch_loan_ead_fully_on_guaranteed_portion(self, sa_results: pl.DataFrame) -> None:
        ...
        assert row["ead_final"] > 0.0, (
            f"P1.229 CRR: guaranteed-portion ead_final should be positive (full "
