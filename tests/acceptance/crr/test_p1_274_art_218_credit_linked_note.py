"""
P1.274 CRR — Art. 218: credit-linked-note own-issuance eligibility gate.
...
is_own_issued_cln attestation:
    ...
    crr_own_issued  — attested issued by the lending institution -> cash
                      treatment (0% haircut), EAD/RWA fall to £500k.
    crr_third_party — own-issuance unattested (null)             -> ineligible,
                      RWA stays at the £1m gross and a CRM019 data-quality
                      warning is raised.

References:
    - CRR Art. 218: own-issued credit-linked note treated as cash collateral.
    - CRR Art. 194(4): funded protection ineligible when materially positively
...
class TestP1274CrrCreditLinkedNote:
    """CRR: a credit-linked note is cash collateral only if attested own-issued."""

    def test_own_issued_cln_reduces_rwa(self) -> None:
        """Control: own-issued CLN gets cash treatment, RWA falls to £500k."""
        result = _run("crr_own_issued")
        assert _rwa(result, SCENARIOS["crr_own_issued"].loan_ref) == pytest.approx(
    ...
    def test_third_party_cln_no_benefit(self) -> None:
        ...
        assert _rwa(result, SCENARIOS["crr_third_party"].loan_ref) == pytest.approx(
            GROSS_RWA, rel=1e-6
        )
    ...
    def test_third_party_rwa_exceeds_own_issued(self) -> None:
        ...
        third_party = _rwa(_run("crr_third_party"), SCENARIOS["crr_third_party"].loan_ref)
        own_issued = _rwa(_run("crr_own_issued"), SCENARIOS["crr_own_issued"].loan_ref)
