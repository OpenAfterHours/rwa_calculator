...
def test_c07_row_0110_contains_row_0120(
    ...
    # Assert
    assert _num(parent, "0200") >= _num(of_which, "0200"), (
        "C 07.00 row 0110 must CONTAIN row 0120 — 0120 is an 'of which' sub-row, not a sibling."
    )
    assert _num(parent, "0220") >= _num(of_which, "0220")
    assert _num(parent, "0200") - _num(of_which, "0200") == pytest.approx(
        expected.netting_set_ead, rel=_REL
    ), "0110 - 0120 must leave exactly the bilateral netting set's exposure value."
    assert _num(parent, "0220") - _num(of_which, "0220") == pytest.approx(
        expected.bilateral_rwea, rel=_REL
